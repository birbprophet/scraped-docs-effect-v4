...
```ts twoslash import.meta.vitest name="managing-a-scope-1"
...
The `Effect.addFinalizer` function is a high-level API that allows you to add finalizers to the scope of an effect. A finalizer is a piece of code that is guaranteed to run when the associated scope is closed. The behavior of the finalizer can vary based on the [Exit](/docs/v4/data-types/exit/) value, which represents how the scope was closed, whether successfully or with an error.
...
```ts twoslash import.meta.vitest name="adding-a-finalizer-on-success-1"
...
```ts twoslash import.meta.vitest name="adding-a-finalizer-on-success-2"
import { Effect, Console, Exit } from "effect"
...
<Aside type="note" title="Finalizer Execution Order">
  Finalizers are executed in reverse order of how they were added, ensuring that
  resources are released in the proper sequence, just like in stack unwinding.
</Aside>
...
```ts twoslash import.meta.vitest name="adding-a-finalizer-on-failure-1"
import { Effect, Console, Exit } from "effect"
...
```ts twoslash import.meta.vitest name="adding-a-finalizer-on-failure-2"
import { Effect, Console, Exit } from "effect"
...
**Example** (Adding a Finalizer on [Interruption](/docs/v4/concurrency/basic-concurrency/#interruptions))

<Tabs syncKey="pipe-vs-gen">

<TabItem label="Using Effect.gen">

```ts twoslash import.meta.vitest name="adding-a-finalizer-on-interruption-1"
import { Effect, Console, Exit } from "effect"
...
```ts twoslash import.meta.vitest name="adding-a-finalizer-on-interruption-2"
import { Effect, Console, Exit } from "effect"
...
```ts twoslash import.meta.vitest name="merging-scopes-1"
...
```ts twoslash import.meta.vitest name="manually-creating-and-closing-scopes-1"
...
<Aside type="note" title="Extending a Scope">
  The `Scope.provide` function allows you to extend the scope of an effect
...
```ts twoslash import.meta.vitest name="closing-a-scope-with-pending-tasks-1"
import { Console, Effect, Exit, Scope } from "effect"
...
```ts twoslash import.meta.vitest name="defining-a-simple-resource-1"
...
```ts twoslash collapse={3-34} import.meta.vitest name="using-the-resource-1"
import { Effect } from "effect"
...
```ts twoslash collapse={3-34} import.meta.vitest name="providing-the-scope-with-effect-scoped-1"
import { Effect } from "effect"
...
To begin, we define the domain model for the required [services](/docs/v4/requirements-management/services/):

- `S3`
- `ElasticSearch`
- `Database`

```ts twoslash import.meta.vitest name="example-pattern-sequencing-operations-1"
...
```ts twoslash collapse={3-48} import.meta.vitest name="example-pattern-sequencing-operations-2"
...
To achieve this, we will utilize [layers](/docs/v4/requirements-management/layers/) to construct test
These layers will be able to handle various scenarios, including errors, which we can control using the `FailureCase` type.

```ts twoslash collapse={3-95} import.meta.vitest name="example-pattern-sequencing-operations-3"
...
In this case, all operations succeed, and we see a `Result.Success` containing the database entry.
...
You can observe that once the `Database` error occurs, there is a complete rollback that deletes the `ElasticSearch` index first and then the associated `S3` bucket. The result is a `Result.Failure` containing the `DatabaseError`.
...
As expected, once the `ElasticSearch` index creation fails, there is a rollback that deletes the `S3` bucket. The result is a `Result.Failure` containing the `ElasticSearchError`.
