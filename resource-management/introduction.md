...
```ts twoslash import.meta.vitest name="running-a-finalizer-in-all-outcomes-1"
...
`Effect.onExit` allows you to run a cleanup effect after the main effect completes, receiving an [Exit](/docs/v4/data-types/exit/) value that describes the outcome.
...
```ts twoslash import.meta.vitest name="running-a-cleanup-function-with-the-effects-result-1"
...
```ts twoslash import.meta.vitest name="running-cleanup-only-on-failure-1"
import { Console, Effect, Exit } from "effect"
...
```ts twoslash import.meta.vitest name="automatically-managing-resource-lifetime-1"
