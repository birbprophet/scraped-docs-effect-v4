...
Use the `Latch.make` function to create a latch in an open or closed state by passing a boolean. The default is `false`, which means it starts closed.
...
```ts twoslash import.meta.vitest name="creating-a-latch-1"
...
const program = Effect.gen(function* () {
  ...
  const fiber = yield* Console.log("open sesame").pipe(
    ...
    Effect.forkChild, // Fork the effect into a new fiber
  )
...
A [semaphore](/docs/v4/concurrency/semaphore/) with one lock (often called a binary semaphore or a mutex) is usually for mutual exclusion: it ensures that only one fiber at a time accesses a shared resource or section of code. Once a fiber acquires the lock, no other fiber can enter the protected area until the lock is released.
