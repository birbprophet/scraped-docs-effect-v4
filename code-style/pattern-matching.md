...
```ts twoslash import.meta.vitest name="handling-different-data-types-with-pattern-matching-1"
...
```ts twoslash import.meta.vitest name="matching-numbers-and-strings-1"
import { Match } from "effect"
...
```ts twoslash import.meta.vitest name="matching-an-object-by-property-1"
import { Match } from "effect"
...
```ts twoslash import.meta.vitest name="matching-with-values-and-predicates-1"
import { Match } from "effect"
...
```ts twoslash import.meta.vitest name="ignoring-a-specific-value-1"
import { Match } from "effect"
...
```ts twoslash import.meta.vitest name="matching-a-discriminated-union-by-tag-1"
import { Match } from "effect"
...
```ts twoslash import.meta.vitest name="using-built-in-predicates-for-property-keys-1"
import { Match } from "effect"
...
```ts twoslash import.meta.vitest name="providing-a-default-value-when-no-patterns-match-1"
import { Match } from "effect"
...
`Match.option` wraps the match result in an [Option](/docs/v4/data-types/option/). If a match is found, it returns `Some(value)`, otherwise, it returns `None`.

**Example** (Extracting a User Role with Option)

```ts twoslash import.meta.vitest name="extracting-a-user-role-with-option-1"
...
The `Match.result` method wraps the result in a [Result](/docs/v4/data-types/result/), providing a structured way to distinguish between matched and unmatched cases. If a match is found, it returns `Success(value)`, otherwise, it returns `Failure(no match)`.

**Example** (Extracting a User Role with Result)

```ts twoslash import.meta.vitest name="extracting-a-user-role-with-result-1"
