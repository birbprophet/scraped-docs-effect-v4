...
- The initial [context](/docs/v4/requirements-management/services/#how-it-works)
- The initial fiber-local state
- The initial effect

It then starts a loop, executing the instructions described by the `Effect` step by step.

You can think of the runtime as a system that takes an [`Effect<A, E, R>`](/docs/v4/getting-started/the-effect-type/) and its associated context `Context<R>` and produces an [`Exit<A, E>`](/docs/v4/data-types/exit/) result.
...
When we use [functions that run effects](/docs/v4/getting-started/running-effects/) like `Effect.runPromise` or `Effect.runFork`, we don't need to mention any runtime object at all. There is no separate "default runtime" value to look up or pass around. These functions execute an `Effect<A, E, never>` directly, using an empty context and the [default services](/docs/v4/requirements-management/default-services/).
...
```ts twoslash import.meta.vitest name="running-synchronously-with-an-explicit-context-1"
...
In most scenarios, this direct style is sufficient for effect execution.
However, there are cases where it's helpful to build a reusable runtime, particularly when you need to reuse specific configurations or contexts across many separate calls.

For example, in a React app or when executing operations on a server in response to API requests, you might build a reusable runtime from a [layer](/docs/v4/requirements-management/layers/) `Layer<R, Err, RIn>` using `ManagedRuntime`, covered next. This allows you to maintain a consistent context across different execution boundaries.
...
```ts twoslash import.meta.vitest name="overriding-the-logger-configuration-1"
...
```ts twoslash import.meta.vitest name="providing-a-configuration-layer-to-a-nested-workflow-1"
...
```ts twoslash import.meta.vitest name="creating-and-using-a-custom-managed-runtime-1"
...
```ts twoslash import.meta.vitest name="defining-a-service-for-notifications-1"
import { Context, Effect } from "effect"
...
Use `.use()` (shown below) to run a callback with the resolved service, or `yield* Notifications` inside `Effect.gen` to access it directly.

This allows you to interact with the service directly:

**Example** (Using the Notifications Service Key)

```ts twoslash import.meta.vitest name="using-the-notifications-service-key-1"
...
```ts twoslash import.meta.vitest name="using-managedruntime-in-an-external-framework-1"
