...
```ts twoslash import.meta.vitest name="how-to-create-a-counter-1"
...
```ts twoslash import.meta.vitest name="how-to-create-a-counter-2"
import { Metric, Effect } from "effect"
...
<Aside type="note" title="Type Preservation">
  Applying a counter to an effect doesn't change its original type. The metric
  simply adds tracking without affecting the effect's output type.
</Aside>
...
```ts twoslash import.meta.vitest name="counter-types-1"
...
```ts twoslash import.meta.vitest name="increment-only-counters-1"
import { Metric, Effect } from "effect"
...
```ts twoslash import.meta.vitest name="counters-with-constant-input-1"
import { Metric, Effect } from "effect"
...
```ts twoslash import.meta.vitest name="how-to-create-a-gauge-1"
import { Metric } from "effect"
...
```ts twoslash import.meta.vitest name="how-to-create-a-gauge-2"
...
<Aside type="note" title="Gauge Behavior">
  Gauges capture the most recent value set, so if you're tracking a sequence of
  updates, the final state will show only the last recorded value, not the
  entire series.
</Aside>
...
```ts twoslash import.meta.vitest name="gauge-types-1"
import { Metric } from "effect"
...
<Aside type="note" title="Histogram Buckets and Precision">
  Keep in mind that histograms don't retain exact values. Instead, they group
  values into buckets, so the precision of your data depends on how you define
  these buckets.
</Aside>
...
```ts twoslash import.meta.vitest name="histogram-1"
...
```ts twoslash import.meta.vitest name="timer-metric-1"
...
```ts twoslash import.meta.vitest name="summary-1"
...
```ts twoslash import.meta.vitest name="frequency-1"
import { Metric, Random, Effect } from "effect"
...
Attributes are key-value pairs you can add to metrics to provide additional context. They help categorize and filter metrics, making it easier to analyze specific aspects of your application's performance or behavior.

Attributes may be attached to one metric or supplied through the context for a region of the program.

### Adding Attributes to a Specific Metric

You can add attributes to an individual metric using `Metric.withAttributes`.

**Example** (Tagging an Individual Metric)

```ts twoslash import.meta.vitest name="adding-attributes-to-a-specific-metric-1"
import { Metric } from "effect"
...
Here, the `request_count` metric has the attribute `"environment": "production"`, allowing you to filter or analyze that metric series later.

### Providing Attributes to a Region

The `Metric.CurrentMetricAttributes` context reference supplies attributes to every metric operation within an effect. This is useful when several metrics share context such as the deployment environment.

**Example** (Tagging Multiple Metrics)

```ts twoslash import.meta.vitest name="providing-attributes-to-a-region-1"
import { Metric, Effect } from "effect"
