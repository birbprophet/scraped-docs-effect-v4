...
The `Schema.revealCodec` function returns the same schema value widened to its full `Codec<T, E, RD, RE>` view, prompting TypeScript to infer all four parameters without any runtime cost.

**Example** (Revealing the Full Codec View)

For example, `Schema.String` has the concrete type `typeof Schema.String`. Passing it to `Schema.revealCodec` exposes its full view as `Codec<string, string, never, never>`.
...
You can create a schema for unique symbols using `Schema.UniqueSymbol`.
...
You can create a union of multiple literals by passing them as arguments to the `Schema.Literals` constructor:
...
You can annotate the entire union to replace its default error message (see [Custom Error Messages](/docs/v4/schema/error-messages#custom-error-messages)).
...
```ts twoslash import.meta.vitest name="exposed-values-1"
import { Schema } from "effect"
...
You can use the `.pick` method of a `Schema.Literals` value to narrow down its possible values.

**Example** (Picking a Subset of Literals)

```ts twoslash import.meta.vitest name="the-pickliteral-utility-1"
import { Schema } from "effect"
...
```ts twoslash import.meta.vitest name="the-pickliteral-utility-2"
import { Schema } from "effect"
...
```ts twoslash import.meta.vitest name="template-literals-2"
import { Schema } from "effect"
...
```ts twoslash import.meta.vitest name="supported-span-types-1"
import { Schema } from "effect"
...
The `Schema.TemplateLiteral` constructor, while useful as a simple validator, only verifies that an input conforms to a specific string pattern by converting template literal definitions into regular expressions. Similarly, [`Schema.isPattern`](/docs/v4/schema/filters#string-filters) uses regular expressions directly for the same purpose. Post-validation, both approaches require additional manual parsing to convert the validated string into a usable data format.
...
```ts twoslash import.meta.vitest name="templateliteralparser-1"
import { Schema } from "effect"
...
The Schema module provides support for native TypeScript enums. You can define a schema for an enum using `Schema.Enum`, allowing you to validate values that belong to the enum.

**Example** (Defining a Schema for an Enum)

```ts twoslash import.meta.vitest name="native-enums-1"
import { Schema } from "effect"
...
```ts twoslash import.meta.vitest name="exposed-values-2"
import { Schema } from "effect"
...
```ts twoslash import.meta.vitest name="union-member-evaluation-order-1"
import { Schema } from "effect"
...
```ts twoslash import.meta.vitest name="union-of-literals-1"
import { Schema } from "effect"
...
You can simplify the process by passing multiple literals directly to the `Schema.Literals` constructor:

**Example** (Defining a Union of Literals)

```ts twoslash import.meta.vitest name="union-of-literals-2"
import { Schema } from "effect"
...
```ts twoslash import.meta.vitest name="nullables-1"
import { Schema } from "effect"
...
```ts twoslash import.meta.vitest name="discriminated-unions-1"
import { Schema } from "effect"
...
```ts twoslash import.meta.vitest name="transforming-a-simple-union-into-a-discriminated-union-1"
import { Schema } from "effect"
...
Here's how you can [transform](/docs/v4/schema/transformations#infallible-transformations) the `Shape` schema into another schema that represents a discriminated union:

**Example** (Adding Discriminant Property)

```ts twoslash import.meta.vitest name="transforming-a-simple-union-into-a-discriminated-union-2"
...
The previous solution works but requires a lot of boilerplate. You can add the discriminant with `mapFields` and use `Schema.tagDefaultOmit` to provide its decoded default while omitting it during encoding:

**Example** (Using `Schema.tagDefaultOmit`)

```ts twoslash import.meta.vitest name="transforming-a-simple-union-into-a-discriminated-union-3"
import { Schema } from "effect"
...
```ts twoslash import.meta.vitest name="exposed-values-3"
import { Schema } from "effect"
...
```ts twoslash import.meta.vitest name="required-elements-1"
import { Schema } from "effect"
...
```ts twoslash import.meta.vitest name="append-a-required-element-1"
import { Schema } from "effect"
...
To define an optional element, use the `Schema.optionalKey` constructor.

**Example** (Defining a Tuple with Optional Elements)

```ts twoslash import.meta.vitest name="optional-elements-1"
import { Schema } from "effect"
...
```ts twoslash import.meta.vitest name="rest-element-1"
import { Schema } from "effect"
...
```ts twoslash import.meta.vitest name="rest-element-2"
import { Schema } from "effect"
...
```ts twoslash import.meta.vitest name="tuples-exposed-values-1"
import { Schema } from "effect"
...
```ts twoslash import.meta.vitest name="mutable-arrays-1"
import { Schema } from "effect"
...
```ts twoslash import.meta.vitest name="exposed-values-4"
import { Schema } from "effect"
...
```ts twoslash import.meta.vitest name="exposed-values-5"
import { Schema } from "effect"
...
```ts twoslash import.meta.vitest name="string-keys-1"
import { Schema } from "effect"
...
```ts twoslash import.meta.vitest name="symbol-keys-1"
import { Schema } from "effect"
...
```ts twoslash import.meta.vitest name="union-of-literal-keys-1"
import { Schema } from "effect"
...
```ts twoslash import.meta.vitest name="template-literal-keys-1"
import { Schema } from "effect"
...
```ts twoslash import.meta.vitest name="refined-keys-1"
import { Schema } from "effect"
...
```ts twoslash import.meta.vitest name="refined-keys-2"
import { Schema } from "effect"
...
you can set [`onExcessProperty`](/docs/v4/schema/getting-started#managing-excess-properties) to `"error"`.
...
<Aside type="note" title="Why Key Transformations Are Not Allowed">
  ...
  multiple keys map to the same value after transformation. To prevent these
  issues, key transformations must be handled explicitly by the user.
</Aside>

To modify record keys, you must apply transformations outside of `Schema.Record`.
A common approach is to use [`Schema.decodeTo`](/docs/v4/schema/transformations#infallible-transformations) with `SchemaTransformation.transform` to adjust keys during decoding.

**Example** (Trimming Keys While Decoding)

```ts twoslash import.meta.vitest name="transforming-keys-1"
...
```ts twoslash import.meta.vitest name="mutable-records-1"
import { Schema } from "effect"
...
```ts twoslash import.meta.vitest name="exposed-values-6"
import { Schema } from "effect"
...
```ts twoslash import.meta.vitest name="property-signatures-1"
import { Schema } from "effect"
...
<Aside type="caution" title="Empty Structs Allow Any Data">
  `Schema.Struct({})` produces the TypeScript type `{}` and accepts any
  non-nullish value. It rejects only `null` and `undefined`.
</Aside>

### Index Signatures

Use `Schema.StructWithRest` to combine a struct with one or more records representing index signatures.

**Example** (Adding an Index Signature)

```ts twoslash import.meta.vitest name="index-signatures-1"
import { Schema } from "effect"
...
```ts twoslash import.meta.vitest name="multiple-index-signatures-1"
import { Schema } from "effect"
...
```ts twoslash import.meta.vitest name="conflicting-index-signatures-1"
import { Schema } from "effect"
...
By using [`Schema.decodeTo`](/docs/v4/schema/transformations#composition) and `SchemaTransformation.transform`,
you can preprocess the input data before decoding. This approach ensures that fixed properties and index signature properties are treated independently.

```ts twoslash import.meta.vitest name="workaround-for-conflicting-index-signatures-1"
import { Schema, SchemaTransformation } from "effect"
...
```ts twoslash import.meta.vitest name="structs-exposed-values-1"
import { Schema } from "effect"
...
```ts twoslash import.meta.vitest name="mutable-structs-1"
...
```ts twoslash import.meta.vitest name="using-the-tag-constructor-1"
import { Schema } from "effect"
...
```ts twoslash import.meta.vitest name="multiple-tags-1"
import { Schema } from "effect"
...
The `Schema.instanceOf` constructor is just a lightweight wrapper of the [Schema.declare](/docs/v4/schema/advanced-usage#declaring-new-data-types) API, which is the primitive in `effect/Schema` for declaring new custom data types.
...
In such cases, you cannot use `Schema.instanceOf`, and you must rely on [Schema.declare](/docs/v4/schema/advanced-usage#declaring-new-data-types) like this:
...
To validate the fields of a class instance, you can use a [filter](/docs/v4/schema/filters/). This approach combines instance validation with additional checks on the instance's fields.
...
Use `Struct.pick` with `Struct.mapFields` to create a new struct schema by selecting fields from an existing one.

**Example** (Picking Properties from a Struct)

```ts twoslash import.meta.vitest name="picking-1"
import { Schema, Struct } from "effect"
...
Use `Struct.omit` with `Struct.mapFields` to create a new struct schema by excluding fields from an existing one.

**Example** (Omitting Properties from a Struct)

```ts twoslash import.meta.vitest name="omitting-1"
import { Schema, Struct } from "effect"
...
Use `Struct.map` with `Schema.optional` to make every field in a struct optional.

**Example** (Making All Properties Optional)

```ts twoslash import.meta.vitest name="partial-1"
import { Schema, Struct } from "effect"
...
`Schema.optional` adds `undefined` to each field type. Use `Schema.optionalKey` instead when a field may be omitted but, when present, must contain a value accepted by its schema.

**Example** (Defining an Exact Partial Schema)

```ts twoslash import.meta.vitest name="partial-2"
import { Schema, Struct } from "effect"
...
Use `Struct.map` with `Schema.requiredKey` to make every optional key in a struct required.

**Example** (Making All Properties Required)

```ts twoslash import.meta.vitest name="required-1"
import { Schema, Struct } from "effect"
