...
| **Cache Inspection**              | Provides functions like `Cache.size`, `Cache.keys`, and `Cache.entries` to inspect the current state of the cache.     |
...
Once a cache is created, the most idiomatic way to work with it is `Cache.get`.
`Cache.get` returns the current value in the cache if it exists, or computes a new value, puts it in the cache, and returns it.
...
```ts twoslash import.meta.vitest name="concurrent-cache-lookups-1"
...
In addition to `Cache.get`, the `Cache` module provides several other functions for working with a cache. Each takes the cache as its first argument, for example `Cache.refresh(cache, key)`:

| Function              | Description                                                                                                                                                                |
| --------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `Cache.refresh`       | Triggers a recomputation of the value for a key without removing the old value, allowing continued access.                                                                 |
| `Cache.size`          | Returns the current size of the cache. The size is approximate under concurrent conditions.                                                                                |
| `Cache.has`           | Checks if a value associated with a specified key exists in the cache. Under concurrent access, the result is valid as of the check time but may change immediately after. |
| `Cache.invalidate`    | Evicts the value associated with a specific key.                                                                                                                           |
| `Cache.invalidateAll` | Evicts all values from the cache.                                                                                                                                          |
