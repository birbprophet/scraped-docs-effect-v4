...
```ts twoslash import.meta.vitest name="recovering-from-failure-1"
...
In this example, `s1` encounters an error, but instead of terminating the stream, we gracefully switch to `s2` using `Stream.catch`. This ensures that we can continue processing data even if one stream fails.

You can also distinguish elements from the two streams based on success or failure by tagging each side with the [Result](/docs/v4/data-types/result/) data type before merging them, mapping `s1`'s elements into `Result.succeed` and `s2`'s elements into `Result.fail`:

```ts twoslash import.meta.vitest name="recovering-from-failure-2"
...
The `Stream.catch` function provides advanced error handling capabilities compared to `Stream.catch`. With `Stream.catch`, you can make decisions based on both the type and value of the encountered failure.

```ts twoslash import.meta.vitest name="recovering-from-failure-3"
import { Stream, Effect } from "effect"
...
```ts twoslash import.meta.vitest name="recovering-from-defects-1"
import { Stream, Effect } from "effect"
...
In this example, `s1` may encounter a defect, but instead of crashing the application, we use `Stream.catchCause` to gracefully switch to an alternative stream, `s2`. This ensures that your application remains robust and continues processing data even in the face of unexpected issues.
...
```ts twoslash import.meta.vitest name="recovery-from-some-errors-1"
...
To recover from a specific cause, you can use the `Stream.catchCauseFilter` function:

```ts twoslash import.meta.vitest name="recovery-from-some-errors-2"
...
```ts twoslash import.meta.vitest name="refining-errors-1"
...
In this example, `stream` initially fails with a generic `Error`. However, `res` filters and keeps only errors of type `SyntaxError`, re-failing the stream with them. Any other error is turned into a defect via `Stream.die`, terminating the stream.
...
```ts twoslash import.meta.vitest name="timeout-1"
import { Stream, Effect } from "effect"
...
```ts twoslash import.meta.vitest name="timeoutfail-1"
...
```ts twoslash import.meta.vitest name="timeoutfailcause-1"
...
```ts twoslash import.meta.vitest name="timeoutto-1"
import { Stream, Effect } from "effect"
