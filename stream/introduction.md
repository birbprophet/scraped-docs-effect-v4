...
```ts twoslash import.meta.vitest name="what-is-a-stream-1"
...
On success, each `Effect` produces exactly one value, even when that value is itself a collection. An effect may instead fail before producing its success value.
...
```ts twoslash import.meta.vitest name="understanding-streams-1"
